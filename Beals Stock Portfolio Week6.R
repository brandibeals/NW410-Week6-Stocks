...
# Sorted barplot
sorted.vv <- returns.cor[order(returns.cor[,21]),21]
barplot(sorted.vv[1:20],las=2,ylim=c(0,1.0), main="Correlations with VV")
...
corrplot(returns.cor, method="shade", shade.col=NA, type="upper", tl.col="black", tl.cex=0.8, diag=FALSE,
         addCoef.col="white", addCoefasPercent=TRUE, number.cex=0.8)
...
cumsum <- cumsum(scree.values) # cumulative sum
eigenvalues <- returns.pca$sdev^2 # eigenvalues
table <- t(cbind(cumsum,eigenvalues))
sum(returns.pca$sdev^2) # total
